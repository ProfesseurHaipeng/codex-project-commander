...
一个跨 Agent 宿主的开源项目总指挥 SKILL：Codex、Claude Code、OpenCode、Kimi Code 与 Hermes 可原生加载；OpenAI、MiniMax、DeepSeek、豆包等模型/API 可通过兼容宿主或便携提示词使用。无论项目刚建立还是运行已久，都先理解项目，再建立组织、账本、Token 路由和连续派工。
...
> 完整模式中的“员工”是宿主真实提供的持久命名项目任务窗口。Codex 桌面端可表现为侧边栏员工；其他平台没有等价工具时自动降级，不用子智能体或虚构角色冒充。
...
- 为每名员工设置当前宿主真实支持的模型与推理基线，并按任务难度动态覆盖。
- 固定设置一名 `员工00｜Token监管与模型路由｜项目名`，专门阻止重复读取、重复派工和模型过度使用。
- 按提供商中立的 Sol、Terra、Luna 三级策略选择最低足够模型，支持 OpenAI、MiniMax、DeepSeek、豆包、Kimi、Hermes 等宿主实际公开的模型目录。
- 把完整任务拆成依赖图，持久记录在 `.codex/project-commander/TASK_LEDGER.md`，随时恢复和核对完成状态。
- 任意员工完成后立即验收并续派下一项适配任务，不等待最慢窗口完成整批工作。
- 首次派工时建立唯一的总指挥任务心跳，主动发现员工完成、读取报告、去重验收并续派；不再假设员工能跨窗口主动汇报。
...
| 平台或提供商 | 原生加载 SKILL | 完整侧边栏员工 | 使用方式 |
| --- | --- | --- | --- |
| OpenAI Codex / Codex | 是 | 有项目任务工具时是 | `.agents/skills` |
| Claude Code（Cloud Code） | 是 | 通常否，降级为会话治理 | `.claude/skills` |
| OpenCode（OpenCloud） | 是 | 按宿主工具判断 | `.opencode/skills` 或 `.agents/skills` |
| Kimi Code | 是 | 按宿主工具判断 | `.kimi-code/skills`、`.agents/skills`、`/skill:project-commander-zh` |
| Hermes Agent | 是 | 默认使用持久会话治理 | GitHub 路径安装 |
| OpenAI API | 否，API 不是宿主 | 否 | 兼容 Agent 宿主或便携提示词 |
| MiniMax / MiniMax Codex | MiniMax 是模型；Codex 是宿主 | 由 Codex 工具决定 | MiniMax 官方 Codex provider + 本 SKILL |
| DeepSeek / DeepSea | 模型/API | 由宿主决定 | OpenAI/Anthropic 兼容宿主 |
| 豆包 / 火山方舟 | 模型/API | 由宿主决定 | OpenAI SDK 兼容宿主 |

完整能力定义、降级规则和 Hermes/Kimi 等入口见 [中文版兼容协议](skills/project-commander-zh/references/platform-compatibility.md)。任何平台都不会因为模型名字自动获得窗口、置顶或心跳能力。
...
### Claude Code、OpenCode、Kimi Code 安装
...
cp -R codex-project-commander/skills/project-commander-zh ~/.config/opencode/skills/project-commander-zh
cp -R codex-project-commander/skills/project-commander-zh ~/.kimi-code/skills/project-commander-zh
```

OpenCode 也会发现 `.agents/skills` 与 `.claude/skills`；Kimi Code 也会发现 `.agents/skills`。在 Kimi Code 中可显式输入 `/skill:project-commander-zh`。
...
如果某个宿主只能接收 system/developer prompt，请使用 [便携中文版提示词](skills/project-commander-zh/assets/PORTABLE_COMMANDER_PROMPT.md)。它提供单会话治理，不会虚构侧边栏员工。
...
交付策略与运行模式独立。部署、发布、上线或生产变更任务若没有说清时间优先级，总指挥只问一次“尽快上线，还是先完成更完整检查？”；用户已经说“先上线”“赶紧部署”时不重复询问。

| 交付策略 | 做法 |
| --- | --- |
| 上线优先 | 通过最小上线门槛后执行已获授权的部署，非阻断项进入上线后补强 |
| 平衡交付 | 完成变更路径检查、构建与关键冒烟后上线 |
| 严格发布 | 高影响或用户明确要求时，先完成更广泛测试、安全、合规或迁移验证 |

上线优先仍不能延期与本次变更有关的密钥泄露、权限、支付、隐私、数据丢失、法律合规、破坏性迁移恢复和关键冒烟问题。除此之外，复用仍有效的证据，只重跑受影响检查，并优先修复前进；小问题不触发反复回滚、全量扫描和重新部署。交付策略词本身不会扩大部署权限。
...
独立任务之间不会自动互相发送消息，桌面通知也不会把员工报告写进总指挥窗口。总指挥会创建一个附着于自身任务的 `总指挥监听｜项目名` 心跳，按节省/中等/效率模式选择低频、正常或快速巡检。它只读取账本中的非终态员工，发现新报告后去重、验收、更新账本并立即续派；队列结束或需要用户决定时暂停。

监听器还会检查看似空闲的窗口是否真的没有工作。它先排除未处理完成、任务合同漏发、仍在运行、待验收和依赖阻塞；存在岗位适配、依赖完成、无写入冲突且 WIP 允许的任务时立即布置。没有这种真实任务时标记休息，不发送待命催促，也不制造凑数任务。

如果当前 Codex 环境没有任务心跳或 Scheduled 能力，总指挥只能在当前轮次保持低频监听，并会明确说明跨轮次自动接力无法保证，不会假装员工能够主动推送报告。
...
完整中文指南见 [Codex便利使用指南](docs/Codex便利使用指南.md) 与 [多平台使用指南](docs/多平台使用指南.md)，内容包括：
...
└── skills/
    ...
    │   ├── assets/PORTABLE_COMMANDER_PROMPT.md
    │   └── references/
