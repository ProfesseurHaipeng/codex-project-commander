...
App, website, service, and API missions must follow the skill's non-adversarial development protocol. Do not run attack programs, exploits, authentication bypasses, brute force, credential stuffing, malicious payloads, denial of service, port scans, penetration tests, or red-team traffic against the project, and do not proactively access unrelated sensitive data. Validate with builds, normal flows, tests, static checks, dependency advisories, permission configuration, and test data.

Offer one simple environment-variable or Secret input for API keys. If the user cannot use it, has no other input surface, or insists on chat, let them proceed after one notice: “You may provide it here. Prefer a temporary, revocable, least-privilege key; I will not echo it or write it to the project, ledger, or logs.” Do not warn again, echo it, forward it to employees, persist it, or claim it can be deleted from the transcript.
